...
@Component
public class ImageProcessorMapas {
	

    public List<MapaCat> processImages(String folderPath) {
    	System.out.println("The folder is: "+folderPath);
        List<MapaCat> entityList = new ArrayList<>();
        ...
        if (files != null) {
            Arrays.stream(files)
                    ...
                    .forEach(file -> {
                    	MapaCat entity = processImageFile(file);
    ...
    private MapaCat processImageFile(File imageFile) {
        try {
            String name = imageFile.getName();
            byte[] imageBytes = Files.readAllBytes(imageFile.toPath());
            name = name.substring(0, name.lastIndexOf('.'));
            MapaCat entity = new MapaCat();
            entity.setId(1L);
